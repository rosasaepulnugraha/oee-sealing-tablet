...
class ListDowntimeDashboard {
  int status;
  final String message;
  final DownTimes? data;

  ListDowntimeDashboard({this.message = "", this.status = 0, this.data});

  factory ListDowntimeDashboard.createListDowntimeDashboard(
      Map<String, dynamic> object, int status) {
    DownTimes? operationList = DownTimes.fromJson(object);
    // List<Plant> plants =
  ...
  static Future<ListDowntimeDashboard> connectToApi(
    ...
    try {
      ...
      return ListDowntimeDashboard.createListDowntimeDashboard(
          jsonObject, apiResult.statusCode);
    } catch (x) {
      log("ERR " + x.toString());
      jsonObject =
          json.decode("{\"status\": 501,\"message\": \"Terjadi kesalahan\"}");
      return ListDowntimeDashboard.createListDowntimeDashboard(jsonObject, 501);
    }
  }
}
